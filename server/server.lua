...
RegisterServerEvent('cw-raidjob:server:givecaseitems', function (jobId)
    local src = source
	local Player = QBCore.Functions.GetPlayer(src)
    if Config.Jobs[jobId].Items == nil then return
    else
    local items = Config.Jobs[jobId].Items

    Player.Functions.RemoveItem("casekey", 1)
    Player.Functions.RemoveItem(items.FetchItem, 1)
    TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items[items.FetchItem], "remove", 1)

    Player.Functions.AddItem(items.FetchItemContents, items.FetchItemContentsAmount)
    TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items[items.FetchItemContents], "add", 1)
    end
end)
...
RegisterServerEvent('cw-raidjob:server:cleanUp', function (jobId)
	...
	TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items[items.FetchItem], "remove", 1)
    TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items["casekey"], "remove", 1)
    end
end)
